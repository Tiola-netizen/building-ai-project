...
def prioritize_tasks(tasks, user_context):
    ...
    for task in tasks:
        ...
        urgency_score = calculate_deadline_urgency(task.deadline)
        energy_match = match_task_to_energy_level(task.complexity, user_context.current_energy)
        context_relevance = assess_context_fit(task, user_context.active_projects)
